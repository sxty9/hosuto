...
// errNotAMember is the closure's way of telling MutateServer "the grant set already excludes this
// person, so write nothing". MutateServer surfaces it verbatim, so revokeUser reports it unchanged.
var errNotAMember = errors.New("that member is not on this server, or was added through a group")
...
//
// The grant filtering runs INSIDE MutateServer, so the whole read-filter-write is one atomic access:
// a grant added on another request while this one is deciding cannot be silently dropped by a write
